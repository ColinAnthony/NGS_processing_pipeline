			...
			for another_string in list(string_dict.keys()):
				if another_string != a_string:
					if a_kmer in string_dict[another_string]:
						unique = False
			if unique is True and min_len <= len(a_kmer) <= max_len:
...
def export_line_to_fastq(fileTag, headerLine, seqLine, plusLine, scoreLine):
	"""
...
def split_by_primers(fastq_file, primer_dict, orientation, make_sure=True):
	"""
	...
	for seq_line in open(fastq_file, 'r'):
		...
		# Get the header
		if is_divisable(line_number_header, 4) is True:
			current_header = seq_line

		# Get the sequence and match to a primer
		if is_divisable(line_number_sequence, 4) is True:
		...
		# Get the plus sign
		if is_divisable(line_number_plus, 4) is True:
			current_plus = seq_line

		# Get the quality scores
		if is_divisable(line_number_quality, 4) is True:
...
def primer_blast_search(db_identifier, sequence):
	"""
	...
	blast_result_raw = Popen(["blastn", "-db", blast_database, "-query", full_directory, "-outfmt", "7", "-word_size", "15", "-evalue", "1000"], stdout=PIPE)
...
if __name__ == '__main__':
	parser = argparse.ArgumentParser(
		description='''A de-multiplexer tool
