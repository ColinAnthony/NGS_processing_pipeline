...
def process_input_dir(fastq_directory_path):
	...
	file_list = os.listdir(fastq_directory_path)
...
def check_for_previous_runs(output_directory_to_check, parsed_input_file_dict):
	...
	if 'setup_log.csv' not in file_list:

		logging.info('No previous runs detected.')

		print('Running first time setup')

		setup_log = open(output_directory_to_check + 'setup_log.csv', 'w')

		setup_log.write('Patient,Sample,R1,R2,status\n')
		...
		for patient in parsed_input_file_dict.keys():
			...
			for sample in parsed_input_file_dict[patient].keys():
				...
				setup_log.write(line_string)

	else:

		print('Reading existing setup file')

		setup_log = open(output_directory_to_check + 'setup_log.csv', 'r')
	...
	setup_log.close()
	return previous_run_dict


def update_setuplog(status_to_update):

	os.rename('setup_log.csv', 'setup_log_old.csv')

	log_file_new = open('setup_log.csv', 'wt')
	log_file_old = open('setup_log_old.csv', 'rt')
...
def main(config_file, output_dir, demultiplex, main_pipeline, haplotype):
	...
	prev_run_dict = check_for_previous_runs(data['input_data']['out_folder'], input_file_dict)
	...
	if demultiplex_fastq:
		...
		for a_patient in input_file_dict.keys():
			...
			for a_sample in input_file_dict[a_patient].keys():
				...
				r1_file_path = infile_directory + input_file_dict[a_patient][a_sample]['R1']
				r2_file_path = infile_directory + input_file_dict[a_patient][a_sample]['R2']
				...
				if fwd_match_only == "yes":
					...
					split_by_primers(r1_file_path, test_primer_dict, 'fwd',
									infast_name, output_dir, a_patient, regex_error_rate, make_sure=should_do_blast)

					infast_name = ntpath.basename(r2_file_path)
					split_by_primers(r2_file_path, test_primer_dict, 'rev',
									infast_name, output_dir, a_patient, regex_error_rate, make_sure=should_do_blast)

		update_complete = update_setuplog('Complete')
		...
		for gene_region, gene_dict in test_primer_dict.items():
			...
			if gene_region is not None or gene_region != "None":
				...
				for a_patient_entry in patient_list:
					...
					path = output_dir + a_patient_entry + '/' + gene_region
